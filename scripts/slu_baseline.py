...
#     Example.word2vec.load_embeddings(model.word_embed, Example.char_vocab, device=device)
# else:
...
def decode(choice):
    ...
    predictions, labels = [], []# labels :[['inform-操作-导航', 'inform-终点名称-贵州省炉盘水市水城县双水六六高速路口'], ['inform-操作-导航', 'inform-终点名称-哈尔滨医科大学附属第一'], ['inform-终点名称-洋浦大街与合肥路交汇', 'inform-操作-导航'], ['inform-操作-导航', 'in
        ...
        for i in range(0, len(dataset), args.batch_size):
            ...
            for j in range(len(current_batch)):
                if any([l.split('-')[-1] not in current_batch.utt[j] for l in pred[j]]):
                    # current_batch.utt[1],pred[1]: ('打开导航我要去林安物流园', ['inform-操作-打开', 'inform-操作-导航', 'inform-终点名称-林安物流园'])
                    # pred相当于预测utt里面的   句子性质-动作类别-动作
                    # 这里就是如果你预测出来的词的动作不在原本场景里，就输出看一下
